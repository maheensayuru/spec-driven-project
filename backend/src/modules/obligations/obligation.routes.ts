...
import { CreateObligationRequestSchema, ListObligationsQuerySchema } from '@renewalradar/shared';
...
export async function obligationRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.patch(
    '/:id',
    { preHandler: [requirePermission('obligations:update')] },
    async (request: FastifyRequest, reply: FastifyReply) => {
      const authReq = request as AuthenticatedRequest;
      const { id } = ParamsWithIdSchema.parse(request.params);
  ...
  server.delete(
    '/:id',
    { preHandler: [requirePermission('obligations:delete')] },
    async (request: FastifyRequest, reply: FastifyReply) => {
      const authReq = request as AuthenticatedRequest;
      const { id } = ParamsWithIdSchema.parse(request.params);
